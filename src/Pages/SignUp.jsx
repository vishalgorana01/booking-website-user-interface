...
function SignUp() {
    ...
    const handleClick = (type) => {
        if (!active.username && type == 'username' && formFields.username == '') {
            usernameRef.current.style.transition = '0.5s'
            usernameRef.current.style.scale = '0.9'
            usernameRef.current.style.transform = 'translateY(-1.5rem)'
            ...
            if (formFields.username == '') {
                usernameRef.current.style.transition = '0.5s'
                usernameRef.current.style.scale = '1'
                usernameRef.current.style.transform = 'translateY(0rem)'
            ...
            <section className='SignInBg flex items-center justify-center w-screen min-h-screen py-12 px-6 sm:px-12'>
                <div className='signInForm flex flex-col items-center gap-y-12 justify-center w-full py-12 max-w-3xl rounded-2xl px-6 sm:px-12' >
                    <span className='flex flex-col items-start text-gray-100 justify-center w-full'>
                        <h1 className='w-full text-left text-2xl '>Create Account</h1>
                        <span className=' w-56 border-blue-500 border-2 rounded-3xl mt-2.5 border-solid mb-3.5'></span>
                    ...
                    <form action="" className='w-full gap-y-4 flex flex-col text-gray-100 items-center justify-center'>
                        <span className='w-full relative py-2 origin-left flex gap-x-3 items-center justify-between z-10' style={{ borderBottom: '2px solid white' }} >
                            <label htmlFor="" className='absolute top-auto left-10 z-10' onClick={() => handleClick('username')} ref={usernameRef}>username</label>
                            <AiOutlineUser className='text-gray-100 text-3xl' />
                            <input type="text" className='w-full outline-none bg-transparent px-2 text-lg text-gray-100' onClick={() => handleClick('username')} name='username' onChange={(ele) => handleChange(ele.currentTarget)} required />
                        </span>
                    ...
                    <span className='flex text-gray-100 items-center justify-between w-full'>
                        ...
                        <button className='text-md py-1.5 px-5 bg-blue-500 rounded-md cursor-pointer hover:bg-blue-400'>Create</button>
                    </span>

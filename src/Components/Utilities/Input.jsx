...
function Input({labelName, price, title}) {
    ...
    return (
        <span className='flex flex-col items-start justify-center gap-y-1.5 w-full'>
            ...
            <input type="number" min={1} name='price' value={state} onChange={(ele) => handleChange(ele.currentTarget.value)} className='outline-none py-0.5 px-1.5 rounded-sm w-full text-[#0000006b]' style={{ border: '2px solid #0000002e' }} />
        </span>
    )
}
